...
class LevinLossMLP:
    ...
    def loss(self, masks, models, trajectory, number_actions, joint_problem_name_list, problem_str, model_num, number_steps):
        """
        ...
        for j in range(len(t) + 1):
            ...
            if j < len(t):
                for i in range(len(masks)):
                    ...
                    if joint_problem_name_list[j] == f"{problem_str}-{model_num}":
                        continue
    ...
    def compute_loss(self, masks, models, problem_str, model_num, trajectories, number_actions, number_steps):
        """
        ...
        for problem, task_trajectories in trajectories.items():
            for model_i, trajectory in enumerate(task_trajectories):
                ...
                name_list = [f"{problem}-{model_i}" for _ in range(len(trajectory._sequence))]
                joint_problem_name_list = joint_problem_name_list + name_list
        return self.loss(masks, models, chained_trajectory, number_actions, joint_problem_name_list, problem_str, model_num, number_steps)
...
def evaluate_all_masks_for_model(masks, selected_models_of_masks, model, problem, model_num, trajectories, number_actions, number_iterations, hidden_size):
    ...
    for value in combinations:
        ...
        value = loss.compute_loss(masks + [current_mask], selected_models_of_masks + [model], problem, model_num, trajectories, number_actions, number_iterations)
    ...
    while previous_loss is None or best_loss < previous_loss:
        ...
        for problem in problems:
            for model_num in range(num_models_per_task):
                ...
                mask, levin_loss = evaluate_all_masks_for_model(selected_masks, selected_models_of_masks, rnn, problem, model_num, trajectories, number_actions, number_iterations, hidden_size)
        ...
        best_loss = loss.compute_loss(selected_masks, selected_models_of_masks, "", -1, trajectories, number_actions, number_iterations)
