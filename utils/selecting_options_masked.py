...
from utils.extract_automaton_cluster import Partitioner, Automaton
...
class LevinLossMLP:
    ...
    def loss(self, masks, options, trajectory, number_actions, joint_problem_name_list, problem_automata):
        """
            ...
            if j < len(t):
                for i in range(len(masks)):
                    ...
                    if joint_problem_name_list[j] == problem_automata:
                        continue
                    finished, actions = options[i].transition(copy.deepcopy(t[j][0]), mask=masks[i])
    ...
    def compute_loss(self, mask, options, problem_automaton, trajectories, number_actions):
        """
        ...
        return self.loss(mask, options, chained_trajectory, number_actions, joint_problem_name_list, problem_automaton)
...
def load_trajectories(problems, hidden_size, game_width):
    ...
    for problem in problems:
        ...
        rnn.load_state_dict(torch.load('binary/game-width' + str(game_width) + '-' + problem + '-rnn-' + str(hidden_size) + '-2-model.pth', weights_only=True))

        trajectory, hidden_state = agent.run(env, rnn, greedy=True)
        trajectories[problem] = trajectory
        hidden_states[problem] = hidden_state

    return trajectories, hidden_states

def evaluate_all_masks_for_model(masks, selected_models_of_masks, option, problem, trajectories, number_actions, hidden_size):
    """
    ...
    combinations1 = []
    combinations2 = []
    for v in itertools.product(values, repeat=hidden_size):
        combinations1.append(v)
    for v in itertools.product(values, repeat=2):
        combinations2.append(v)

    for value1 in combinations1:
        for value2 in combinations2:
            current_mask1 = torch.tensor(value1, dtype=torch.int8).view(1, -1)
            current_mask2 = torch.tensor(value2, dtype=torch.int8).view(1, -1)
            current_mask = [current_mask1, current_mask2]
            # print(masks, current_mask)
            value = loss.compute_loss(masks + [current_mask], selected_models_of_masks + [option], problem, trajectories, number_actions)
...
def evaluate_all_masks_levin_loss():
    ...
    trajectories, hidden_states = load_trajectories(problems, hidden_size, game_width)
    ...
    while previous_loss is None or best_loss < previous_loss:
        ...
        for problem in problems:
            ...
            env = ComboGridEnv(game_width, game_width, problem)
            option = Automaton(kmeans, hidden_states[problem], rnn)
            mask, levin_loss = evaluate_all_masks_for_model(selected_masks, selected_models_of_masks, option, problem, trajectories, number_actions, hidden_size)

            if best_loss is None or levin_loss < best_loss:
                best_loss = levin_loss
                best_mask = mask
                model_best_mask = option
        ...
        best_loss = loss.compute_loss(selected_masks,selected_models_of_masks, "", trajectories, number_actions)
    ...
    loss = LevinLossMLP()
    # loss.print_output_subpolicy_trajectory(selected_models_of_masks, selected_masks, selected_options_problem, trajectories)

    # printing selected options
    with open("selected_options.txt", 'w') as file:
        for i in range(len(selected_masks)):
            print(selected_masks[i])
            file.write(f"{selected_masks[i]}\n")
